...
void frontpage()
{
	...
	char * str = (char*)"WELCOME TO";
	bitmap_output(width/3+50, height-height/10, str,GLUT_BITMAP_TIMES_ROMAN_24);
	...
	str = (char*)"Prims Algorithm Visualiser";
	bitmap_output(width/3+10, height-height/6, str,GLUT_BITMAP_TIMES_ROMAN_24);
	glColor3f(0.9,0.8,0.0);
	...
	delay();
	glColor3f(1,1,0.2);
	str = (char *)"Press Enter to Continue.......";
	bitmap_output(width/2+150, 50,str,GLUT_BITMAP_HELVETICA_18);
	glFlush();
	delay();
	glutPostRedisplay();
}

//this function displays text instructions in grpahics window
void Instructions()
{
	glColor3f(1.0,1.0,0.0);
	char *str = (char *)"Instructions";
	bitmap_output(10, height-height/10, str,GLUT_BITMAP_TIMES_ROMAN_24);
	...
	str = (char*)"1) Press N to Create Nodes";
	bitmap_output(10, height-height/7, str,GLUT_BITMAP_HELVETICA_18);
	str = (char *)"2) Press E to draw Edges";
	bitmap_output(10,height-height/5.8, str,GLUT_BITMAP_HELVETICA_18);
	str = (char *)"3) Press D to draw spanning tree";
	bitmap_output(10, height-height/4.9, str,GLUT_BITMAP_HELVETICA_18);
}
...
void output()
{
	...
	str = (char *)"Spanning tree exist and minimum spanning tree is : ";
	bitmap_output(xaxis,yaxis,str,GLUT_BITMAP_HELVETICA_18);
...
void display(){
	...
	if(load<800 && found==0){
			glColor3f(1,1,1.0);
			str = (char *)"LOADING...";
			bitmap_output(100,325,str,GLUT_BITMAP_HELVETICA_18);
		    loadpage();
		}
	else if(load<800 && found==1){
	        glColor3f(1,1,1.0);
			str = (char *)"CALCULATING....";
			bitmap_output(100,325,str,GLUT_BITMAP_HELVETICA_18);
		    loadpage();
	}else{
	if(!enterpressed){
...
void input(){
	...
	int posi=-1,posj=-1;
	int cost=0;
	int center_difference_error = 15;
	for(int i=0;i<pointer;i++){
		if(oldx[i][0]>=(linepoints[0][0]-center_difference_error)  &&
		   oldx[i][0]<=(linepoints[0][0]+center_difference_error))
		{
			if(oldx[i][1]>=(linepoints[0][1]-center_difference_error)
				&&oldx[i][1]<=(linepoints[0][1]+center_difference_error))
			{
		       posi=i;
			   break;
			}
	...
	for(int i=0;i<pointer;i++)
	{
		if(oldx[i][0]>=(linepoints[1][0]-center_difference_error) &&
		   oldx[i][0]<=(linepoints[1][0]+center_difference_error))
		{
			if(oldx[i][1]>=(linepoints[1][1]-center_difference_error) && 
			   oldx[i][1]<=(linepoints[1][1]+center_difference_error))
			{
		       posj=i;
			   break;
			}
	...
	if((posi==posj)||posi==-1||posj==-1){
		glColor3f(1.0,1.0,0.0);
		str = (char *)"Invalid Node";
		bitmap_output(20,50,str,GLUT_BITMAP_HELVETICA_18);
		glFlush();
		for(int i=0; i<3; i++)
			delay();
...
void find(){
	...
	if(isconnected()){
	 ...
	 {
		glColor3f(1.0,1.0,0.0);
		str = (char *)"Graph is not connected !";
		bitmap_output(30,20,str,GLUT_BITMAP_HELVETICA_18);
		glFlush();
...
void find_spanning_tree(){
	...
	//d[] stores distance from source of all other vertices
	//p[] stores the parent of ith vertex
	for(int i=0;i<pointer;i++)
	{
	...
	//bcz MST has (v-1) edges so 1 to V
	for(int i=1;i<pointer;i++)
	{
		...
		//include it to mst and then itrate to the adjacent edges(updation)
		s[u]=1;
...
int main(int argc,char** argv){
	...
	//OpenGL window intialization and setting callback functions
	glutInit(&argc,argv);
...
// g++ Prims_Visualizer.cpp -lGL -lGLU -lglut
